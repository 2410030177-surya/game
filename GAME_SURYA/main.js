...
function handleRockDestruction(rock, fireball, fireballIndex, rockIndex) {
    ...
    if (rock.isDestroyed()) {
        ...
        if (rock.fragmentLevel === 0) {
            ...
            // MEDIUM ROCK → Split into 2 SMALL rocks
            fragmentRock(rock, rockIndex);
        } else {
            // SMALL ROCK (level 2) → Final disintegration with dust/embers
...
function restartGame() {
    ...
    // Hide cursor during gameplay for immersive experience
    const canvas = document.getElementById('gameCanvas');
    if (canvas) {
...
// ============================================
// MOUSE CLICK DETECTION - For Restart Button
// ============================================

// Add click event listener to canvas for restart button
window.addEventListener('click', (e) => {
    // Only process clicks when game is over
    if (!gameState.isGameOver) {
        return; // Ignore clicks during gameplay
    }
    ...
    // Calculate mouse position relative to canvas (not page)
    const mouseX = e.clientX - rect.left; // X position inside canvas
    const mouseY = e.clientY - rect.top;  // Y position inside canvas
    
    // Define restart button boundaries (must match drawGameOver function)
    // Calculate position dynamically to match rendering
    const scorePanelY = CONFIG.canvas.height / 2 + 20;
    const scorePanelHeight = 180;
    const buttonX = CONFIG.canvas.width / 2 - 160; // Left edge of button
    const buttonY = scorePanelY + scorePanelHeight + 25; // Top edge of button
    const buttonWidth = 320;  // Button width
    const buttonHeight = 55;  // Button height
    
    // Check if click is within button boundaries (rectangle collision detection)
    const clickedButton = mouseX >= buttonX && // Mouse is right of left edge
                         mouseX <= buttonX + buttonWidth && // Mouse is left of right edge
                         mouseY >= buttonY && // Mouse is below top edge
                         mouseY <= buttonY + buttonHeight; // Mouse is above bottom edge
    
    // If button was clicked, restart the game
    if (clickedButton) {
        console.log('🖱️ Restart button clicked!');
        restartGame(); // Calls the same function as 'R' key press
    }
});
...
window.addEventListener('mousemove', (e) => {
    if (!gameState.isGameOver) {
        mouseX = getCanvasPosition(e); // Update mouse position
    }
});
...
window.addEventListener('mousedown', (e) => {
    if (!gameState.isGameOver) {
        isTouching = true;
        mouseX = getCanvasPosition(e); // Set initial position
    }
});
...
// Touch Move - Finger drags across screen
...
// Touch End - Finger lifts off screen
